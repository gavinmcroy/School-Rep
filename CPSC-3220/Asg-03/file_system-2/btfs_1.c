...
unsigned int btfs_check_fd_in_range(unsigned int fd) {
...
unsigned int btfs_check_block_in_range(unsigned int b) {
    if ((b < 2) || (b >= N_BLOCKS)) {
...
unsigned int btfs_check_file_is_open(unsigned int fd) {
...
unsigned int btfs_check_valid_name(char *name) {
...
unsigned int btfs_new_directory_entry() {
    unsigned int fd;
...
unsigned int btfs_map_name_to_fd(char *name) {
    unsigned int fd;
    if (!btfs_check_valid_name(name)) return (0);
...
unsigned int btfs_exists(char *name) {
    if (!btfs_check_valid_name(name)) return (FALSE);
    if (btfs_map_name_to_fd(name) == 0) return (FALSE);
    return (TRUE);
}
...
unsigned int btfs_create(char *name) {
    unsigned int file_descriptor;
    if (!btfs_check_valid_name(name)) return (0);
    if (btfs_map_name_to_fd(name) != 0) return (0);
    file_descriptor = btfs_new_directory_entry();
...
unsigned int btfs_open(char *name) {
    unsigned int file_descriptor;
    if (!btfs_check_valid_name(name)) return (0);
    file_descriptor = btfs_map_name_to_fd(name);
    if (file_descriptor == 0) return (0);
    if (btfs_check_file_is_open(file_descriptor)) return (0);
    directory[file_descriptor].status = OPEN;
    directory[file_descriptor].byte_offset = 0;
    return (file_descriptor);
}
...
unsigned int btfs_close(unsigned int file_descriptor) {
    if (!btfs_check_fd_in_range(file_descriptor)) return (FALSE);
    if (!btfs_check_file_is_open(file_descriptor)) return (FALSE);
...
unsigned int btfs_size(unsigned int file_descriptor) {
    if (!btfs_check_fd_in_range(file_descriptor)) return (MAX_FILE_SIZE + 1);
...
unsigned int btfs_seek(unsigned int file_descriptor, unsigned int offset) {
    if (!btfs_check_fd_in_range(file_descriptor)) return (FALSE);
    if (!btfs_check_file_is_open(file_descriptor)) return (FALSE);
