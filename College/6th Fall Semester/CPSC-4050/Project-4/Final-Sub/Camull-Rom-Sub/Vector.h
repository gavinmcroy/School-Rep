#ifndef __VZL_VECTOR_H__
#define __VZL_VECTOR_H__
...
public:
    ...
    const Vector operator*(const double v) const { return Vector(xyz[0] * v, xyz[1] * v, xyz[2] * v); }
    ...
    const double magnitude() const { return sqrt(xyz[0] * xyz[0] + xyz[1] * xyz[1] + xyz[2] * xyz[2]); }
...
//  Comparisons
    ...
    const bool operator<(const Vector &v) const { return (magnitude() < v.magnitude()); }
    ...
    char *__str__() const {
        ...
        std::sprintf(tmp, "(%g,%g,%g)", xyz[0], xyz[1], xyz[2]);
        return tmp;
    }
