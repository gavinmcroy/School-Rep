 ...
 *     btfs_size(), a return value > MAX_FILE_SIZE is used to
 *     indicate an error)
 *
...
struct directory_entry {
    ...
    /* after the changes it was 30 bytes, so I added this to make it 32 */
...
unsigned int btfs_seek(unsigned int file_descriptor,
                       unsigned int offset);

unsigned int btfs_read(unsigned int file_descriptor,
                       char *buffer,
                       unsigned int byte_count);

unsigned int btfs_write(unsigned int file_descriptor,
                        char *buffer,
                        unsigned int byte_count);

unsigned int btfs_close(unsigned int file_descriptor);

unsigned int btfs_delete(unsigned int file_descriptor);
