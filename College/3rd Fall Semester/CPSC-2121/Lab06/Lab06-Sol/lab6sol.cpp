...
struct Student {
  ...
  double cutoff; // point in time by which certain fraction of productivity is spent
  double tstart, tstop; // time range to be assigned to this student
};
...
double get_halfway_cutoff(Student &s, double t1, double t2)
{
  ...
  double total = get_productivity(s, t1, t2), lower = t1, upper = t2, mid;
  while (upper - lower > 0.0001) {
    mid = (lower + upper) / 2;
    if (get_productivity(s, t1, mid) < total / 2) lower = mid;
    else upper = mid;
  }
  return lower;
}
...
void partition(int s1, int s2, double val)
{
  int i=s1, j=s2;
  while (i < j) {
    if (S[i].cutoff <= val) { i++; continue; }
    if (S[j].cutoff > val) { j--; continue; }
    swap (S[i++], S[j--]); 
  }
  if (S[s1].cutoff > val) return;
  i=j=s1;
  while (j < s2 && S[j+1].cutoff <= val) j++;
  while (i < j) {
    if (S[i].cutoff < val) { i++; continue; }
    if (S[j].cutoff == val) { j--; continue; }
    swap (S[i++], S[j--]); 
...
{
  int r = s2-s1+1; // # of elements in S[s1..s2]
  double pivot_value = S[s1 + rand()%r].cutoff; // pick random pivot value
  //  for (int i=s1; i<=s2; i++) cout << S[i].cutoff << " ";
  //  cout << " -- partition on " << pivot_value << "\n";
  partition(s1, s2, pivot_value);
  for (r=0; S[s1+r].cutoff<pivot_value; r++); // find pivot location
  //  for (int i=s1; i<=s2; i++) cout << S[i].cutoff << " ";
  //  cout << " -- r = " << r << "\n";
  if (rank == r) return pivot_value;
  if (rank <  r) return quickselect (s1, s1+r-1, rank);
  return                quickselect (s1+r+1, s2, rank - r - 1);
}
...
void solve(int s1, int s2, double t1, double t2)
{
  ...
  // function for partitioning you developed for quickselect
...
void test_quickselect(void)
{
  ...
  for (int j=0; j<N; j++) {
    ...
    cout << "\nshuffle(A[0..6]) is ";
    for (int i=0; i<N; i++) cout << i << ": " << S[i].cutoff << " ";
    cout << "\n";
