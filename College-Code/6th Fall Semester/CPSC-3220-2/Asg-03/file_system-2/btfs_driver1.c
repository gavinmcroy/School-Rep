...
int main() {
    struct directory_entry directoryEntry;
    ...
    fd[0] = btfs_create("file.txt");
    if (fd[0] == 0) printf("first create failed\n");

    fd[1] = btfs_create("my_file");
    if (fd[1] == 0) printf("second create failed\n");

    btfs_list_directory();

    count1 = btfs_write(fd[0], buffer1, length1);
    printf("%d bytes written to first file\n", count1);

    count2 = btfs_write(fd[1], buffer2, length2);
    ...
    btfs_seek(fd[0], 600);
    count3 = btfs_read(fd[0], buffer3, 640);
    printf("%d bytes read from first file\n", count3);
    buffer3[count3] = '\0';
    printf("[%s]\n", buffer3);

    btfs_seek(fd[0], 250);
    count3 = btfs_read(fd[0], buffer3, 20);
    printf("%d bytes read from first file\n", count3);
    buffer3[count3] = '\0';
    printf("[%s]\n", buffer3);

    fd[2] = btfs_create("file.txt");
    printf("fd for creating a file with identical name");
    printf(" as existing file - %d\n", fd[2]);
    fd[2] = btfs_create("file3");
    fd[4] = btfs_create("file4");
    fd[4] = btfs_create("file5");
    fd[5] = btfs_create("file6");
    fd[6] = btfs_create("file7");
    fd[7] = btfs_create("file8");
    fd[8] = btfs_create("file9");
    fd[9] = btfs_create("file10");
    fd[10] = btfs_create("file11");
    fd[11] = btfs_create("file12");
    fd[12] = btfs_create("file13");
    fd[13] = btfs_create("file14");
    fd[14] = btfs_create("file15");
    fd[15] = btfs_create("file16");
    ...
    fd[16] = btfs_create("added_1");
    fd[17] = btfs_create("added_2");
