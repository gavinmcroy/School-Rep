...
Node *filter_evens(Node *root)
{
  ...
  root->left = filter_evens(root->left);
  root->right = filter_evens(root->right);
  fix_size(root);
  if (root->key%2 == 1) {
    result = join(root->left, root->right);
    delete root;
  }
  return result;
}
...
int main(void)
{
  Node *T = NULL;
  for (int i=0; i<20; i++)
    T = insert_keep_balanced(T, rand() % 1000);
  print_inorder(filter_evens(T));
  return 0;
}
