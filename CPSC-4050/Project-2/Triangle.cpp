...
/*  inside outside test for our triangle */
    ...
    vzl::Vector cross0 = point - vertex0;
    vzl::Vector cross1 = point - vertex1;
    vzl::Vector cross2 = point - vertex2;

    if (!((normal * (edge0 ^ cross0)) > 0 && (normal * (edge1 ^ cross1)) > 0 &&
          (normal * (edge2 ^ cross2)) > 0)) {
        return NO_INTERSECTION;
    }
