...
std::string fileOutputName = "out.png";
    ...
    auto dimx = imagePlane.getNX(), dimy = imagePlane.getNY();
 
   
    ofstream ofs;
    ofs.open("first.ppm", ios_base::out| ios_base::binary);
    ...
    for (auto j = 0u; j < dimy; ++j)
        for (auto i = 0u; i < dimx; ++i){
          int address = (j * imagePlane.getNX() + i) * numChannels;
            ofs <<                 
                /* We need to multiply by maxPixVal to scale pixel value up from 0.0-1.0 to 0-255 */
                imageData[address] << endl <<
                         // inputImageData[address + x];
                imageData[address + 1] << endl <<
                        

                imageData[address + 2] <<endl;
             }
