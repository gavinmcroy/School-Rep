...
//   Color.h
//
// 4D color class in the namespace vzl
//
...
#ifndef __VZL_COLOR_H__
#define __VZL_COLOR_H__
...
//! a class to hold 4-component float color values.
    ...
    public:
        ...
        //! Unary minus
        friend const Color operator-(const Color &v) { return Color(-v.xyzw[0], -v.xyzw[1], -v.xyzw[2], -v.xyzw[3]); }

        //! Multiplication of a constant with a Color
        friend const Color operator*(const float w, const Color &v) { return v * w; }

        //! Multiplication of a Color with a constant
        const Color operator*(const float v) const { return Color(xyzw[0] * v, xyzw[1] * v, xyzw[2] * v, xyzw[3] * v); }
