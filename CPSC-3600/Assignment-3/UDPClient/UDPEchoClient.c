...
int main(int argc, char *argv[]) {
  ...
  if (sock < 0)
    ...
    // Create a big string and send the entire thing over
    char *inputBuffer = (char *) calloc(1024,sizeof(char));
    char *mainString = (char *) calloc(65536,sizeof(char));
    while (fgets(inputBuffer,256,file) != NULL) {
        strcat(mainString, inputBuffer);
        inputBuffer[0] = '\0';
        strcat(mainString, " ");
    }
    free(inputBuffer);
    fclose(input);
    echoString = mainString;
    echoStringLen = strlen(mainString);
