...
//    for (int x = 0; x < blurImage.getWidth(); x++) if (blurImage.getPixel(x, 0) == blurImage.red) return true;
//    return false;
//}
//
//void remove_seam(void) {
//    cout << "Removing seam to decrease width to " << blurImage.getWidth() - 1 << "\n";
//    for (int y = 0; y < blurImage.getHeight(); y++) {
//        int where_red = -1;
//        for (int x = 0; x < blurImage.getWidth(); x++)
//            if (blurImage.getPixel(x, y) == blurImage.red)
//                if (where_red != -1) {
...
//            exit(0);
//        }
//        for (int x = where_red; x < blurImage.getWidth() - 1; x++) blurImage.getPixel(x, y) = blurImage.getPixel(x + 1, y);
//    }
//
//    // Create a new image with one smaller width
//    Blur::Pixel *orig_image = blurImage.getImage();
//    blurImage.changeWidth(1);
//    width--;
...
int main(int argc, char *argv[]) {
    Blur blur;
    //blur.readImage("billboard.ppm");
    blur.commenceApplication(argc,argv);
