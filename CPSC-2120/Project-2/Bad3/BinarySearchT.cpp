...
BinarySearchT::Node *BinarySearchT::insertOnRank(BinarySearchT::Node *root, keyType k, int rank) {
    if (root == nullptr) {
        return new Node(rank);
    }
    //root->size++;
    if (root->size >= rank) {
        root->left = insertOnRank(root->left, k, rank);
    } else {
        root->right = insertOnRank(root->right, k, rank - 1);
    }
