...
void processKeystrokes() {
    ...
    while (ch != '\n') {
        ...
        for (unsigned i = 0; i < everyDistinctWordVec.at(wordIndex).pages.size(); i++) {
...
//                    }
                ...
                if (it != pages.at(pageIndex).words.end()) {
                    unsigned int location = it - pages.at(pageIndex).words.begin();
...
int main() {
    ...
    for (unsigned int i = 0; i < everyDistinctWordVec.at(myLookUp).pages.size(); i++) {
