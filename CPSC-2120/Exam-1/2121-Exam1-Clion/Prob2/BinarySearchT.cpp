...
BinarySearchT::Node *BinarySearchT::insert(Node *root, keyType k,  const std::string& s) {
    if (root == nullptr)
        return new Node(k, s);
    if (k < root->key)
        root->left = insert(root->left, k, s);
    else
        root->right = insert(root->right, k, s);
    update_size(root);
    return root;
}
...
BinarySearchT::Node *BinarySearchT::insert_keep_balanced(Node *root, keyType k,  const std::string& s) {
    if (root == nullptr) return new Node(k, s);
    if (rand() % (1 + root->size) == 0) {
        Node *new_root = new Node(k, s);
        tie(new_root->left, new_root->right) = split(root, k);
        update_size(new_root);
        return new_root;
    }
    if (k < root->key) root->left = insert_keep_balanced(root->left, k, s);
    else root->right = insert_keep_balanced(root->right, k, s);
    update_size(root);
    return root;
}
