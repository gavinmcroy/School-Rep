...
int main( int argc, char *argv[] ) {
	...
	if ( argc == 2 ) { 
		...
		for ( i=0; i<num_urls; i++ ) {
			...
			// the entire program.
			if(pthread_create(&(thread_array[i]), NULL, (void*)th_run, &(index[i])) != 0){
				exit(0);
			} 
			else{
				//don't do anything because threads were made 
			}
		...
		for(int j = 0; j < num_urls; j++){
			if(pthread_join(thread_array[j], NULL) != 0){
				...
				//don't do anything threads were joined 
			}
		}
		...
		char* fileName = "make_merica_great_again_.txt";
...
int write_url_file( char* file_path, web_t* web_array, int num_urls ) {
	...
	//Creating the file and pointers 
	FILE* myFile;
	...
	//for every single url find the link
	for(int i = 0; i < num_urls; i++){
		int urlCount = web_array[i].link_cnt;
		
		for(int j = 0; j < urlCount; j++){
			//url 
			website = web_array[i].url;
			//links
			url = web_array[i].links[i];
			
			//looking for trump
			if(strstr(url, "trump") != NULL){
				ssize_t lineSize = strlen(url) + strlen(url) + 5;
				char strLine[lineSize];
				
				//skip www. 
				website += 4;
				
				//string for file 
				strcpy(strLine, website);
				strcat(strLine, ", ");
				strcat(strLine, url);
				strcat(strLine, "\n");
				
				fputs(strLine, myFile);
			}
		}
	} 
	fclose(myFile);
