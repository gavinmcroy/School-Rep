...
public class GameSetupController {
    ...
    public void processButtonClick(int rows, int cols, int players, int numWin) {
        String errorMsg = "";
        if (rows < IGameBoard.MIN_BOARD_SIZE || rows > max_size) {
            errorMsg += "Rows must be between " + IGameBoard.MIN_BOARD_SIZE + " and " + max_size;
        }

        if (cols < IGameBoard.MIN_BOARD_SIZE || cols > max_size) {
            errorMsg += "Columns must be between " + IGameBoard.MIN_BOARD_SIZE + " and " + max_size;
        }
